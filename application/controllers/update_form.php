...
class Update_form extends CI_Controller
{
	...
	public function profile()
	{
		...
		$content = array('page_content' => $this->m_display->programType(),
						 'page_view_content' => $this->m_view->profile($empID));
		// echo "<pre>";
		// print_r($content);
		// echo "</pre>";
		$this->load->view('includes/header');
	...
	{
		$educID = $this->uri->segment(3, 0);	
 		$content = array('page_view_content' => $this->m_view->faculty_eligibility($educID));
 		// echo "<pre>";
		// print_r($content);
		// echo "</pre>";
		$this->load->view('includes/header');
		$this->load->view('includes/navi_faculty');
		$this->load->view('update_eligibility', $content);
		$this->load->view('includes/footer');
	}
